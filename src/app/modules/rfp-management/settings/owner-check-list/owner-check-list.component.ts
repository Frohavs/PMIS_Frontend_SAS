...
@Component({
  selector: 'app-owner-check-list',
  templateUrl: './owner-check-list.component.html',
  styleUrl: './owner-check-list.component.scss'
})
export class OwnerCheckListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  initRfpList(pageIndex?: number, search?: string) {
    this.dataList = [];
    this.rfpManagementService.getAllOwnerCheck(pageIndex, search).subscribe(res => {
  ...
  redirectToNew() {
    this.router.navigate(['rfp_management/add-owner-check']);
  }
