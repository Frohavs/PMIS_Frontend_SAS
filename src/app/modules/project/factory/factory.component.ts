...
export class FactoryComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  constructor(
    ...
    this.Add_text = this.translate.instant('FACTORY.Add_Factory');
  ...
  initFactoryList(id: number, pageIndex?: number, search?: string) {
    this.dataList = [];
    this.factoryService.getAll(id, pageIndex, search).subscribe(res => {
  ...
  redirectToNew() {
    this.router.navigateByUrl('projects/add-factory/' + this.projectId)
  }

  factoryDetails(factory: any) {
    this.router.navigate(['projects/factory-details/' + this.projectId], {
