...
export class VisitFormComponent implements OnInit, AfterViewInit, OnDestroy{
  projectId: number;

  Add_text: string = 'new visit';
  Search_text: string = 'search visits';
  dataList: any[] = [];
  totalCount: number;
  pagesCount: number[] = [];
  selected = 1;
  ...
  initScheduleList(id: number, pageIndex?: number, search?: string) {
    this.dataList = [];
    this.visitFormService.getAll(id, pageIndex, search).subscribe(res => {
  ...
  redirectToNew() {
    this.router.navigateByUrl('projects/add-visit/' + this.projectId)
  }

  editVisitForm(id: number) {
    this.router.navigateByUrl('projects/edit-visit/' + this.projectId + '?visitId=' + id)
  }

  redirectToDetails(visit: any) {
    this.router.navigate([`projects/visit-details/${this.projectId}`], {
      queryParams: { visitId: visit.id }
    });
  }
  ...
  deleteSite(user: any) {
    this.deleteSwal.fire().then((clicked) => {
      if (clicked.isConfirmed) {
        this.isLoading = true;
        this.visitFormService.deleteSite(user.id).subscribe({
          next: (res) => {
