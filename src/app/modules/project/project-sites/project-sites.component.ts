...
export class ProjectSitesComponent implements OnInit, AfterViewInit, OnDestroy{
  projectId: number;

  Add_text: string = 'new site';
  Search_text: string = 'search sites';
  dataList: any[] = [];
  totalCount: number;
  pagesCount: number[] = [];
  selected = 1;
  ...
  initScheduleList(id: number, pageIndex?: number, search?: string) {
    this.dataList = [];
    this.projectSitesService.getAll(id, pageIndex, search).subscribe(res => {
